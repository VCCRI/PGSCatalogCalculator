...
getPlots <- function(inFiles, inRecord){
  ...
  lPLots <- lapply(list(unique(disData$PGS_RECORD_ID)), function(x){
    ...
    require(cowplot)
    theme_set(theme_cowplot())
    return(ggplot(data.frame(orDF, stringsAsFactors=F), aes(x=splitNo, y=oddsRatio)) +
    geom_pointrange(aes(ymin=lowerBound, ymax=upperBound)) + ggtitle(x))
  })
  return(lPLots)
}
...
setPlots <- function(inFiles){
  ...
  require(ggplot2)
  require(cowplot)
  ggsave(filename=inFile, plot=cowplot::plot_grid(plotlist=allPlots))
  #dev.off()
}
