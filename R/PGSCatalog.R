...
getPGSProfileHasControl <- function(inPGS){
  return(inPGS@hasControl)
}
getPGSProfileMidSamples <- function(inPGS){
  ...
  ##while(!(resolved(inspecFile) & resolved(normFile))) {}
     ...
     scoreFiles <- lapply(inputFile, function(inObjec){
      ...
      if(inObjec@`pgsId` %in% file | is.na(file)){
        ...
        scoreFiles <- foreach::foreach(d=iterators::iter(unique(combFrame),by='row'), .export = c("getPGSType","filterBCFCalc", "filterBCFCalcPos", "getChrPos", "getGRSInputChrPos", "getGRSInputRsID", "getGRSInputRsID", "getRSIds", "getPGSId", "filterMerged", "filterMergedPos", "filterMergedReg", "getMakePlink", "plinkGRS", "baseIndex", "makeFamFile", "getMergePlink", "inControl", "inYamlFile"), .packages=c("data.table", "yaml", "assertthat")) %dopar% {
